const account = {
  account: [
    {
      id: "136848375",
      nickname: "-",
    },
    {
      id: "137040004",
      nickname: "-",
    },
    {
      id: "437603645",
      nickname: "-",
    },
    {
      id: "139183663",
      nickname: "-",
    },
    {
      id: "141035590",
      nickname: "-",
    },
    {
      id: "144446422",
      nickname: "-",
    },
    {
      id: "144842125",
      nickname: "-",
    },
    {
      id: "144853334",
      nickname: "-",
    },
    {
      id: "146442178",
      nickname: "-",
    },
    {
      id: "147103127",
      nickname: "-",
    },
    {
      id: "149669698",
      nickname: "-",
    },
    {
      id: "151331280",
      nickname: "-",
    },
    {
      id: "155184820",
      nickname: "-",
    },
    {
      id: "164531657",
      nickname: "-",
    },
    {
      id: "164771913",
      nickname: "-",
    },
    {
      id: "164806205",
      nickname: "-",
    },
    {
      id: "165162169",
      nickname: "-",
    },
    {
      id: "174735319",
      nickname: "-",
    },
    {
      id: "437981172",
      nickname: "-",
    },
    {
      id: "205181448",
      nickname: "-",
    },


    {
      id: "437403576",
      nickname: "-",
    },
    {
      id: "208486130",
      nickname: "-",
    },
    {
      id: "208505789",
      nickname: "-",
    },
    {
      id: "208520455",
      nickname: "-",
    },
    {
      id: "209801013",
      nickname: "-",
    },
    {
      id: "216232854",
      nickname: "-",
    },
    {
      id: "217278677",
      nickname: "-",
    },
    {
      id: "437189095",
      nickname: "-",
    },
    {
      id: "443006414",
      nickname: "-",
    },
    {
      id: "231115687",
      nickname: "-",
    },
    {
      id: "23:6997401",
      nickname: "-",
    },
    {
      id: "237006820",
      nickname: "-",
    },
    {
      id: "238207345",
      nickname: "-",
    },
    {
      id: "241363223",
      nickname: "-",
    },
    {
      id: "241365105",
      nickname: "-",
    },
    {
      id: "241367942",
      nickname: "-",
    },
    {
      id: "248520170",
      nickname: "-",
    },
    {
      id: "258146625",
      nickname: "-",
    },
    {
      id: "443224723",
      nickname: "-",
    },
    {
      id: "443500824",
      nickname: "-",
    },
    {
      id: "261216173",
      nickname: "-"
    },
    {
      id: "261613546",
      nickname: "-"
    },
    {
      id: "424525159",
      nickname: "-"
    },
    {
      id: "274897779",
      nickname: "-"
    },
    {
      id: "276108850",
      nickname: "-"
    },
    {
      id: "436276472",
      nickname: "-"
    },
    {
      id: "435363526",
      nickname: "-"
    },
    {
      id: "284606508",
      nickname: "-"
    },
    {
      id: "292042051",
      nickname: "-"
    },
    {
      id: "295547860",
      nickname: "-"
    },
    {
      id: "299903334",
      nickname: "-"
    },
    {
      id: "300470447",
      nickname: "-"
    },
    {
      id: "309804201",
      nickname: "-"
    },
    {
      id: "311235215",
      nickname: "-"
    },
    {
      id: "312831524",
      nickname: "-"
    },
    {
      id: "313236114",
      nickname: "-"
    },
    {
      id: "314200479",
      nickname: "-"
    },
    {
      id: "318879325",
      nickname: "-"
    },
    {
      id: "337862970",
      nickname: "-"
    },
    {
      id: "341246350",
      nickname: "-"
    },
    {
      id: "45414828",
      nickname: "-"
    },
    {
      id: "436297710",
      nickname: "-"
    },
    {
      id: "50146642",
      nickname: "-"
    },
    {
      id: "439653168",
      nickname: "-"
    },
    {
      id: "324830382",
      nickname: "-"
    },
    {
      id: "305927196",
      nickname: "-"
    },
    {
      id: "320333369",
      nickname: "-"
    },
    {
      id: "315373260",
      nickname: "-"
    },
    {
      id: "310222075",
      nickname: "-"
    },
    {
      id: "310267712",
      nickname: "-"
    },
    {
      id: "306112869",
      nickname: "-"
    },
    {
      id: "300343789",
      nickname: "-"
    },
    {
      id: "295695376",
      nickname: "-"
    },
    {
      id: "322148032",
      nickname: "-"
    },
    {
      id: "314596116",
      nickname: "-"
    },
    {
      id: "310451490",
      nickname: "-"
    },
    {
      id: "308182185",
      nickname: "-"
    },
    {
      id: "305350772",
      nickname: "-"
    },
    {
      id: "299082589",
      nickname: "-"
    },
    {
      id: "391606785",
      nickname: "-"
    },
    {
      id: "272110030",
      nickname: "-"
    },
    {
      id: "47146466",
      nickname: "-"
    },
    {
      id: "440120149",
      nickname: "-"
    },
    {
      id: "441669176",
      nickname: "-"
    },
    {
      id: "323766660",
      nickname: "-"
    },
    {
      id: "323810247",
      nickname: "-"
    },
    {
      id: "323817154",
      nickname: "-"
    },
    {
      id: "323823294",
      nickname: "-"
    },
    {
      id: "323787552",
      nickname: "-"
    },
    {
      id: "323655055",
      nickname: "-"
    },
    {
      id: "323712952",
      nickname: "-"
    },
    {
      id: "323801263",
      nickname: "-"
    },
    {
      id: "32:3829648",
      nickname: "-"
    },
    {
      id: "323751839",
      nickname: "-"
    },
    {
      id: "323729568",
      nickname: "-"
    },
    {
      id: "369228986",
      nickname: "-"
    },
    {
      id: "375562733",
      nickname: "-"
    },
    {
      id: "442205390",
      nickname: "-"
    },
    {
      id: "440682845",
      nickname: "-"
    },
    {
      id: "300752755",
      nickname: "-"
    },
    {
      id: "300743458",
      nickname: "-"
    },
    {
      id: "324144860",
      nickname: "-"
    },
    {
      id: "372778837",
      nickname: "-"
    },
    {
      id: "309855376",
      nickname: "-"
    },
    {
      id: "309953986",
      nickname: "-"
    },
    {
      id: "309716747",
      nickname: "-"
    },
    {
      id: "311160656",
      nickname: "-"
    },
    {
      id: "311727459",
      nickname: "-"
    },
    {
      id: "313140193",
      nickname: "-"
    },
    {
      id: "314591065",
      nickname: "-"
    },
    {
      id: "314684122",
      nickname: "-"
    },
    {
      id: "255528102",
      nickname: "-"
    },
    {
      id: "325554284",
      nickname: "-"
    },
    {
      id: "300731040",
      nickname: "-"
    },
    {
      id: "300728304",
      nickname: "-"
    },
    {
      id: "300736109",
      nickname: "-"
    },
    {
      id: "321470120",
      nickname: "-"
    },
    {
      id: "3:27513745",
      nickname: "-"
    },
    {
      id: "355139031",
      nickname: "-"
    },
    {
      id: "245929223",
      nickname: "-"
    },

    {
      id: "437855669",
      nickname: "-",
    },
    {
      id: "438184259",
      nickname: "-",
    },
    {
      id: "436705492",
      nickname: "-",
    },
    {
      id: "315505260",
      nickname: "-",
    },
    {
      id: "312118807",
      nickname: "-",
    },
    {
      id: "313283961",
      nickname: "-",
    },
    {
      id: "313353889",
      nickname: "-",
    },
    {
      id: "372688023",
      nickname: "-",
    },
    {
      id: "365484303",
      nickname: "-",
    },
    {
      id: "260267978 M2004J19C",
      nickname: "-",
    },
    {
      id: "175903919",
      nickname: "-",
    },
    {
      id: "208203184",
      nickname: "-",
    },
    {
      id: "188799028",
      nickname: "-",
    },
    {
      id: "247636646",
      nickname: "-",
    },
    {
      id: "244836099",
      nickname: "-",
    },
    {
      id: "216550187",
      nickname: "-",
    },
    {
      id: "312115795",
      nickname: "-",
    },
    {
      id: "312112483",
      nickname: "-",
    },
    {
      id: "315459795",
      nickname: "-",
    },
    {
      id: "216479578",
      nickname: "-",
    },
    {
      id: "216242749",
      nickname: "-",
    },
    {
      id: "216438394",
      nickname: "-",
    },
    {
      id: "216330265",
      nickname: "-",
    },
    {
      id: "272455321",
      nickname: "-",
    },
    {
      id: "456842745",
      nickname: "-",
    },
    {
      id: "451941201",
      nickname: "-",
    },
    {
      id: "487363665",
      nickname: "-",
    },
    {
      id: "403519265",
      nickname: "-",
    },
    {
      id: "403936535",
      nickname: "-",
    },
    {
      id: "402918410",
      nickname: "-",
    },
    {
      id: "403367651",
      nickname: "-",
    },
    {
      id: "283528567",
      nickname: "-",
    },
    {
      id: "282710692",
      nickname: "-",
    },
    {
      id: "279533801",
      nickname: "-",
    },
    {
      id: "246949087",
      nickname: "-",
    },
    {
      id: "261485355",
      nickname: "-",
    },
    {
      id: "203089495",
      nickname: "-",
    },
    {
      id: "283386393",
      nickname: "-",
    },
    {
      id: "179530014",
      nickname: "-",
    },
    {
      id: "179983025",
      nickname: "-",
    },
    {
      id: "322041928",
      nickname: "-",
    },
    {
      id: "300370454",
      nickname: "-",
    },
    {
      id: "378882063",
      nickname: "-",
    },
    {
      id: "376297538",
      nickname: "-",
    },
    {
      id: "360868045",
      nickname: "-",
    },
    {
      id: "373222571",
      nickname: "-",
    },
    {
      id: "373229106",
      nickname: "-",
    },
    {
      id: "373249167",
      nickname: "-",
    },
    {
      id: "380953121",
      nickname: "-",
    },
    {
      id: "399333577",
      nickname: "-",
    },
    {
      id: "121337822",
      nickname: "-",
    },
    {
      id: "525133406",
      nickname: "-",
    },
    {
      id: "403779463",
      nickname: "-",
    },
    {
      id: "403737426",
      nickname: "-",
    },
    {
      id: "404842746",
      nickname: "-",
    },
    {
      id: "3:89767756",
      nickname: "-",
    },
    {
      id: "379131526",
      nickname: "-",
    },
    {
      id: "402906549",
      nickname: "-",
    },
    {
      id: "530223539",
      nickname: "-",
    },
    {
      id: "410644509",
      nickname: "-",
    },
    {
      id: "405009545",
      nickname: "-",
    },
    {
      id: "407259515",
      nickname: "-",
    },
    {
      id: "408222603",
      nickname: "-",
    },
    {
      id: "300350129",
      nickname: "-",
    },
    {
      id: "407808121",
      nickname: "-",
    },
    {
      id: "406337596",
      nickname: "-",
    },
    {
      id: "409609766",
      nickname: "-",
    },
    {
      id: "410477088",
      nickname: "-",
    },
    {
      id: "405993204",
      nickname: "-",
    },
    {
      id: "414826910",
      nickname: "-",
    },
    {
      id: "413923361",
      nickname: "-",
    },
    {
      id: "413937473",
      nickname: "-",
    },
    {
      id: "413980106",
      nickname: "-",
    },
    {
      id: "413985138",
      nickname: "-",
    },
    {
      id: "414027936",
      nickname: "-",
    },
    {
      id: "414952656",
      nickname: "-",
    },
    {
      id: "415211290",
      nickname: "-",
    },
    {
      id: "415626332",
      nickname: "-",
    },
    {
      id: "415748404",
      nickname: "-",
    },
    {
      id: "416306966",
      nickname: "-",
    },

    {
      id: "416674481",
      nickname: "-"
    },
    {
      id: "417207657",
      nickname: "-"
    },
    {
      id: "419013447",
      nickname: "-"
    },
    {
      id: "416272441",
      nickname: "-"
    },
    {
      id: "415153900",
      nickname: "-"
    },
    {
      id: "421942069",
      nickname: "-"
    },
    {
      id: "423146987",
      nickname: "-"
    },
    {
      id: "180771855",
      nickname: "-"
    },
    {
      id: "427432209",
      nickname: "-"
    },
    {
      id: "426434748",
      nickname: "-"
    },
    {
      id: "172464551",
      nickname: "-"
    },
    {
      id: "422297421",
      nickname: "-"
    },
    {
      id: "420510798",
      nickname: "-"
    },
    {
      id: "413834540",
      nickname: "-"
    },
    {
      id: "413463600",
      nickname: "-"
    },
    {
      id: "413081727",
      nickname: "-"
    },
    {
      id: "162551680",
      nickname: "-"
    },
    {
      id: "408592275",
      nickname: "-"
    },
    {
      id: "403117082",
      nickname: "-"
    },
    {
      id: "402030947",
      nickname: "-"
    },
    {
      id: "401070613",
      nickname: "-"
    },
    {
      id: "203823371",
      nickname: "-"
    },
    {
      id: "476655847",
      nickname: "-"
    },
    {
      id: "502640598",
      nickname: "-"
    },
    {
      id: "382617441",
      nickname: "-"
    },
    {
      id: "511433113",
      nickname: "-"
    },
    {
      id: "369858447",
      nickname: "-"
    },
    {
      id: "428840421",
      nickname: "-"
    },
    {
      id: "495470892",
      nickname: "-"
    },
    {
      id: "486632712",
      nickname: "-"
    },
    {
      id: "490276603",
      nickname: "-"
    },
    {
      id: "483178304",
      nickname: "-"
    },
    {
      id: "470443794",
      nickname: "-"
    },
    {
      id: "461683554",
      nickname: "-"
    },
    {
      id: "465740030",
      nickname: "-"
    },
    {
      id: "469223356",
      nickname: "-"
    },
    {
      id: "465096564",
      nickname: "-"
    },
    {
      id: "481331716",
      nickname: "-"
    },
    {
      id: "441567152",
      nickname: "-"
    },
    {
      id: "451808875",
      nickname: "-"
    },
    {
      id: "523137038",
      nickname: "-"
    },
    {
      id: "520996569",
      nickname: "-"
    },
    {
      id: "518106606",
      nickname: "-"
    },
    {
      id: "518122375",
      nickname: "-"
    },
    {
      id: "520972327",
      nickname: "-"
    },
    {
      id: "517227585",
      nickname: "-"
    },
    {
      id: "516091357",
      nickname: "-"
    },
    {
      id: "513041907",
      nickname: "-"
    },
    {
      id: "370966009",
      nickname: "-"
    },
    {
      id: "360939010",
      nickname: "-"
    },
    {
      id: "429677005",
      nickname: "-"
    },
    {
      id: "369781006",
      nickname: "-"
    },
    {
      id: "385597956",
      nickname: "-"
    },
    {
      id: "428984784",
      nickname: "-"
    },
    {
      id: "505635057",
      nickname: "-"
    },
    {
      id: "463718768",
      nickname: "-"
    },
    {
      id: "441863496",
      nickname: "-"
    },
    {
      id: "529265625",
      nickname: "-"
    },
    {
      id: "466168047",
      nickname: "-"
    },
    {
      id: "514687228",
      nickname: "-"
    },
    {
      id: "432661296",
      nickname: "-"
    },
    {
      id: "528295639",
      nickname: "-"
    },
    {
      id: "365773189",
      nickname: "-"
    },
    {
      id: "528514074",
      nickname: "-"
    },
    {
      id: "472491906",
      nickname: "-"
    },
    {
      id: "473859158",
      nickname: "-"
    },
    {
      id: "481831398",
      nickname: "-"
    },
    {
      id: "473754278",
      nickname: "-"
    },
    {
      id: "419679960",
      nickname: "-"
    },
    {
      id: "381122194",
      nickname: "-"
    },
    {
      id: "360788396",
      nickname: "-"
    },
    {
      id: "363584839",
      nickname: "-"
    },
    {
      id: "279798711",
      nickname: "-"
    },
    {
      id: "206792334",
      nickname: "-"
    },
    {
      id: "98633329",
      nickname: "-"
    },
    {
      id: "367354112",
      nickname: "-"
    },
    {
      id: "315569088",
      nickname: "-"
    },
    {
      id: "279715418",
      nickname: "-"
    },
    {
      id: "57644033",
      nickname: "-"
    },
    {
      id: "374822065",
      nickname: "-"
    },
    {
      id: "470986653",
      nickname: "-"
    },
    {
      id: "520148954",
      nickname: "-"
    },
    {
      id: "522857829",
      nickname: "-"
    },
    {
      id: "523023540",
      nickname: "-"
    },
    {
      id: "526696386",
      nickname: "-"
    },
    {
      id: "527807774",
      nickname: "-"
    },
    {
      id: "528623432",
      nickname: "-"
    },
    {
      id: "528974122",
      nickname: "-"
    },
    {
      id: "529494259",
      nickname: "-"
    },
    {
      id: "520143861",
      nickname: "-"
    },
    {
      id: "474010198",
      nickname: "-"
    },

  ],
};

const tableAccount = document.getElementById("table-account");

for (let i = 0; i < account.account.length; i++) {
  const id = account.account[i].id;
  const nickname = account.account[i].nickname;
  const row = document.createElement("tr");
  const colCheck = document.createElement("td");
  const divContainerCheck = document.createElement("div");
  const divCheck = document.createElement("div");
  const check = document.createElement("input");

  const colId = document.createElement("td");
  const divId = document.createElement("div");
  const pId = document.createElement("p");

  const colNick = document.createElement("td");
  colNick.setAttribute("class", "");

  const colNumber = document.createElement("td");
  colNumber.setAttribute("class", "");

  const divNick = document.createElement("div");
  divNick.setAttribute("class", "flex items-center");

  const divNumber = document.createElement("div");
  divNumber.setAttribute("class", "flex items-center");

  const pNick = document.createElement("p");
  pNick.setAttribute("class", "text-sm leading-none text-gray-600 ml-2");

  const pNumber = document.createElement("p");
  pNumber.setAttribute("class", "text-sm leading-none text-gray-600 ml-2");

  divId.setAttribute("class", "flex items-center pl-5");
  pId.setAttribute("class", "text-base leading-none text-gray-700 mr-2");

  const colCopy = document.createElement("td");
  colCopy.setAttribute("class", "flex flex-row justify-end mr-2 py-2");

  const btnCopy = document.createElement("button");
  btnCopy.setAttribute(
    "class",
    "ml-auto focus:ring-2 focus:ring-offset-2 focus:ring-pink-300 text-sm leading-none text-gray-600 py-3 px-5 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none"
  );
  btnCopy.setAttribute("id", "btn-copy" + i);

  btnCopy.innerHTML = "Copy";

  colCopy.appendChild(btnCopy);

  divNick.appendChild(pNick);
  colNick.appendChild(divNick);


  pNumber.innerHTML = i + 1;

  divNumber.appendChild(pNumber);
  colNumber.appendChild(divNumber);


  pId.innerHTML = id;
  divId.appendChild(pId);
  colId.appendChild(divId);

  check.setAttribute("type", "checkbox");
  check.setAttribute("id", "check" + i);
  check.setAttribute("name", "check" + i);
  check.setAttribute(
    "class",
    "focus:opacity-100 checkbox absolute cursor-pointer w-full h-full"
  );

  divContainerCheck.setAttribute("class", "ml-5");
  divCheck.setAttribute(
    "class",
    "bg-gray-200 rounded-sm w-5 h-5 flex flex-shrink-0 justify-center items-center relative"
  );

  divCheck.appendChild(check);
  divContainerCheck.appendChild(divCheck);

  row.setAttribute("tabindex", i);
  row.setAttribute("id", "row" + i);
  row.setAttribute("class", "focus:outline-none rounded");
  colCheck.appendChild(divContainerCheck);
  colNick.innerHTML = nickname;
  row.appendChild(colNumber);
  row.appendChild(colId);
  row.appendChild(colNick);
  row.appendChild(colCheck);
  row.appendChild(colCopy);
  tableAccount.appendChild(row);

  const isChecked = document.getElementById("check" + i);

  btnCopy.addEventListener("click", () => {
    navigator.clipboard.writeText(id);
    isChecked.checked = true;
    document.getElementById("row" + i).classList.add("bg-pink-200");
  });

  isChecked.addEventListener("change", () => {
    document.getElementById("row" + i).classList.remove("bg-pink-200");
  });
}
